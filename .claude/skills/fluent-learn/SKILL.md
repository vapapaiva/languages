...
Check `computed.orphaned_partials[]` — if a prior session was interrupted, offer commit-or-discard before opening a new session_id (see `fluent-db-updater`).
...
- 1-5 → hand off to the matching skill (`fluent-writing`, `fluent-speaking`, `fluent-vocab`, `fluent-reading`, `fluent-review`). Those skills cover everything needed; this skill's job here is just to dispatch. **The sub-skill owns checkpoint + commit — `fluent-learn` writes nothing in this branch.**
- 6 (adaptive mix) → use this skill's own exercise sequencer (below). Checkpoint after each exercise; commit once at session end.
...
### 8. Per-answer feedback + checkpoint

Use `fluent-feedback-formatter` template. Score 0-10 + severity tag.

**Then `--checkpoint`** with the running `skill_scores` (across every skill touched by this adaptive mix), plus any new `review_results[]` / `errors[]` / `new_vocabulary[]`:

```bash
python3 .claude/hooks/update-db.py --checkpoint <<'EOF'
{
  "session_id": "session-013", "date": "2026-05-27",
  "command_used": "/fluent-learn",
  "skills_practiced": ["vocabulary", "grammar"],
  "skill_scores": {
    "vocabulary": {"exercises": 4, "correct": 4, "time_minutes": 3},
    "grammar": {"exercises": 2, "correct": 1, "time_minutes": 4}
  },
  "review_results": [...],
  "errors": [...]
}
...
If option 6 (adaptive mix), do **one** `--commit` with the session-final fields. If we delegated to a sub-skill in step 4, that sub-skill already committed — `fluent-learn` does NOT call commit a second time.
...
python3 .claude/hooks/update-db.py --commit <<'EOF'
{
  ...
  "breakthroughs": [...],
  "focus_next_session": [...],
  "session_notes": "Adaptive mix — covered omdat word order + uit/van prepositions."
}
EOF
```
...
- **Routing rule for DB writes:** if we hand off to a sub-skill (options 1-5), the sub-skill owns checkpoint + commit. If we run adaptive mix (option 6) inline, this skill owns both. **Never** double-write.
