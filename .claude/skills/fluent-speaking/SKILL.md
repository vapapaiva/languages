...
**Then `--checkpoint`** — running `skill_scores.speaking` + any communication-blocking error. Use the **turn number as part of `pattern_id`** (e.g. `speaking_turn3_word_order`) so revisions to the same turn overwrite cleanly rather than create duplicates.

```bash
python3 .claude/hooks/update-db.py --checkpoint <<'EOF'
{
  "session_id": "session-011", "date": "2026-05-27",
  "command_used": "/fluent-speaking",
  "skill_scores": {"speaking": {"exercises": 3, "correct": 2, "time_minutes": 8}},
  "errors": [{"pattern_id": "speaking_turn2_uit_van", "category": "grammar",
              "your_answer": "...", "correct_answer": "...", "severity": "minor"}]
}
EOF
```
...
Per-turn data is already in the partial. Single `--commit`:
...
python3 .claude/hooks/update-db.py --commit <<'EOF'
{
  ...
  "duration_minutes": 18,
  "command_used": "/fluent-speaking",
  "topics_covered": ["weekend plans", "ordering food"],
  "focus_next_session": ["..."]
}
EOF
```
...
- **Checkpoint after every turn, commit once at end.** Use turn-number-tagged `pattern_id` so revisions stay idempotent.
