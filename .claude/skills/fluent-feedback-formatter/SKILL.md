...
### 1. Standard template — single answer
...
### 1b. Batched template — N atomic answers in one table

When the upstream skill batched N atomic exercises (vocab review, cloze, MC, single-sentence error correction) into one prompt, the learner answers all of them in one message. Grade them in **one combined table**, one row per item:

```markdown
## 📋 Feedback — {N} items

| # | Prompt | Your answer | Verdict | Score |
|---|--------|-------------|---------|-------|
| 1 | el mercado | рынок | ✅ | 10/10 |
| 2 | el ayuntamiento | мерия | ✅ (typo: **мэрия**) | 9/10 |
| 3 | cercano | рядом | 🔴 **близлежащий (прил.)** — "рядом" = cerca (adv) | 4/10 |
...

**Batch score: {X}/{N} correct ({percent}%)**

**Notes:**
- 🟢 {minor pattern}
- 🟡 {moderate pattern, possibly recurring — flag if in mistakes-db}
```

Rules for the batched table:
- Severity emoji on every ❌ row (🔴 / 🟡 / 🟢) — same scale as single-answer feedback.
- Show the correction inline in the Verdict column.
- One score per row out of 10, plus a batch-level summary.
- The "Notes" block calls out cross-cutting patterns the learner should focus on next — especially if a mistake matches an existing entry in `mistakes-db.error_patterns` (mention by name).
- Each row still feeds one `review_results[]` entry; the upstream skill checkpoints once after the table is rendered.
...
- **Use the right template.** Single-answer template (§1) for one constructed answer. Batched table (§1b) when the upstream skill presented N atomic exercises in one prompt. Never mix the two for the same batch.
- **Always use the chosen template exactly.** Deviations break session-file parsing downstream.
