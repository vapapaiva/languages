...
**Do NOT checkpoint yet.** Wait until rewrite-or-skip is resolved — a rewrite revises the canonical score and the set of errors. Checkpointing twice would either double-count errors (if not last-write-wins) or leave stale ones in the partial. The merge semantics use last-write-wins by `pattern_id`, so a clean re-write that simply omits an error from the second checkpoint will NOT erase it — you must include the corrected (or removed-from-final) state explicitly.

The safe pattern: after step 7 resolves (whether rewrite or skip), write **one** `--checkpoint` with the final set of `errors[]` + final `skill_scores.writing`.
...
Single `--commit` (the per-session checkpoint from step 7 already holds the graded data):

```bash
python3 .claude/hooks/update-db.py --commit <<'EOF'
{
  "session_id": "session-012", "date": "2026-05-27",
  "duration_minutes": 16,
  "command_used": "/fluent-writing",
  "skills_practiced": ["writing"],
  "focus_next_session": ["omdat word order", "formal closing"]
}
EOF
```
...
- **Checkpoint AFTER the rewrite-or-skip decision resolves.** Don't checkpoint after the first scoring — a rewrite revises the final state. One checkpoint, then commit.
