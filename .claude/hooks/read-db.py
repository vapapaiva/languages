...
def main():
    ...
    # Detect orphaned partials — sessions that were checkpointed but never committed.
    # Skills should surface these on startup and offer commit-or-discard.
    orphaned_partials = []
    if PARTIALS_DIR.exists():
        for p in sorted(PARTIALS_DIR.glob("*.partial.json")):
            try:
                with open(p, 'r', encoding='utf-8') as f:
                    part = json.load(f)
                orphaned_partials.append({
                    "session_id": part.get("session_id", p.stem.replace(".partial", "")),
                    "date": part.get("date", ""),
                    "command_used": part.get("command_used", ""),
                    "started_at": part.get("started_at", ""),
                    "last_checkpoint_at": part.get("last_checkpoint_at", ""),
                    "review_results_count": len(part.get("review_results_by_id", {})),
                    "new_vocabulary_count": len(part.get("new_vocabulary_by_id", {})),
                    "errors_count": len(part.get("errors_by_id", {})),
                    "is_stale": part.get("date", "") < today,
                    "path": str(p),
                })
            except (json.JSONDecodeError, OSError):
                # Don't fail the whole read if one partial is corrupt
                continue
